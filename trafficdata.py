...
# RAM optimization for render

def optimize_dataframe_memory(df):
    print("--- DEBUG: Starting memory optimization ---")
    initial_memory = df.memory_usage(deep=True).sum() / (1024**2)
    print(f"--- DEBUG: Initial DataFrame memory usage: {initial_memory:.2f} MB ---")
    ...
    for col in df.columns:
        ...
        if col_type == object:
            ...
            if num_unique_values / num_total_values < 0.5:
                ...
                print(f"--- DEBUG: Column '{col}' converted to category.")
            ...
            print(f"--- DEBUG: Column '{col}' downcasted to {df[col].dtype}.")
        elif 'float' in str(col_type):
            df[col] = df[col].astype(np.float32)
            print(f"--- DEBUG: Column '{col}' downcasted to {df[col].dtype}.")

    final_memory = df.memory_usage(deep=True).sum() / (1024**2)
    print(f"--- DEBUG: Final DataFrame memory usage: {final_memory:.2f} MB (Reduced by {(initial_memory - final_memory)/initial_memory*100:.2f}%) ---")
    print("--- DEBUG: Memory optimization finished ---")
    return df
...
def load_models():
    return joblib.load("avg_speed_model.pkl"), joblib.load("volume_model.pkl")
...
speed_model, volume_model = load_models()
...
with st.spinner("Loading dashboard..."):
    ...
    with tab2:
        ...
        st.subheader("Average Number of Vehicles per Month")
        vehicles_per_month_df = vehicles_per_month.reset_index()
        fig3 = px.line(
            vehicles_per_month_df,
            x='Month',
            y='Number_of_Vehicles',
            labels={'Month': 'Month', 'Number_of_Vehicles': 'Vehicles'},
        ...
        for df in [top_fast, top_slow, median_roads, least_busy, busiest, median_busy]:
            ...
            # First Graphs - Speed and Traffic Summaries
        st.subheader("Top Roads by Speed and Volume")
        ...
        axs[0, 2].set_title('Median Roads (Middle 5)')
        ...
        st.divider()
            # Third Graph - Bar Plots by Road Type
    ...
    with tab7:
        ...
        def traffic_calendar(df):
            ...
            selected_year = st.selectbox("Select a Year:", sorted(df['Vehicle_Date'].dt.year.dropna().astype(int).unique()),key='year')
            if selected_year is None: 
                st.warning("No valid years available in the data to select for the calendar.")
                return
            ...
            for i, month in enumerate(range(1, 13)):
                ...
                for week_num, week in enumerate(cal):
                    for day_idx, day in enumerate(week):
                        if day == 0:
                            continue
                        date_obj = pd.Timestamp(year=int(selected_year), month=month, day=day).date()
    ...
    with tab10:
        ...
        def create_speed_df():
            ...
            df_input['Road_Type'] = df_input['Road_Number'].apply(classify_road_type)
            return df_input
